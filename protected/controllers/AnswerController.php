...
class AnswerController extends Controller
{
	...
	public function actionCreate($question_id=null)
	{
        ...
        if(!$question_id || !is_int($question_id))
        {
            throw new CHttpException(400, 'Не указан номер вопроса для создаваемого ответа!');
            Yii::app()->end();
        }
	...
	public function actionDelete($id)
	{
            ...
            $question_id = $this->loadModel($id)->question_id;
			...
			if(!isset($_GET['ajax']))
                ...
                $this->redirect(array('index', 'question_id'=>$question_id));
	 ...
	 * Lists all models.
	 */
	public function actionIndex($question_id=null)
	{
        $question_id = $this->castToInt($question_id);
    ...
    private function  castToInt($param)
    {
        $param = (int)$param;
        if(!$param || !is_int($param))
        {
            throw new CHttpException(400, 'Не передан правильный параметр');
        }
        return $param;
    }
}
